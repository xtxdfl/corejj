...
export default observer(function () {
  ...
  const [pushKey, setPushKey] = useState(store.settings.corejj_push_key);
  ...
  function handleBind() {
    ...
    setLoading(true);
    http.post('/api/setting/push/bind/', {corejj_push_key: pushKey})
      .then(res => {
  ...
  function handleUnbind() {
    ...
    setLoading(true);
    http.post('/api/setting/push/bind/', {corejj_push_key: ''})
      .then(() => {
  ...
  const corejjPushKey = store.settings.corejj_push_key
  return (
    <Spin spinning={fetching}>
      ...
      <div style={{maxWidth: 340}}>
        <Form.Item label="推送助手账户绑定" labelCol={{span: 24}} style={{marginTop: 12}}
                   extra={<div>请登录推送助手，至个人中心 / 个人设置查看用户ID，注意保密该ID请勿泄漏给第三方。<Link
                     href="https://push.corejj.cc/guide/corejj" title="配置手册"/></div>}>

          {corejjPushKey ? (
            <Input.Group compact>
              <div className={css.keyText}
                   style={{width: 'calc(100% - 100px)', lineHeight: '32px', fontWeight: 'bold'}}>{corejjPushKey}</div>
      ...
      {balance.vip_desc ? (
        <Form.Item style={{marginTop: 24}}
                   extra={<div> 如需充值请至 <Link href="https://push.corejj.cc/buy/sms" title="推送助手"/>，具体计费规则及说明请查看推送助手官网。
                   </div>}>
            ...
            <div className={css.body}>
              ...
              <div className={css.item}>
                ...
                {isVip ? (
                  ...
                  <Link href="https://push.corejj.cc/buy/vip" title={`订阅会员每天赠送${balance.mail_free}封`}
                        className={css.tips}/>
                )}
              </div>
              <div className={css.item}>
                ...
                {isVip ? (
                  ...
                  <Link href="https://push.corejj.cc/buy/vip" title={`订阅会员每天赠送${balance.wx_mp_free}条`}
                        className={css.tips}/>
                )}
              </div>
              <Link href="https://push.corejj.cc/buy/vip" className={css.badge} title={balance.vip_desc}/>
            </div>
          </div>
        </Form.Item>
      ) : null}

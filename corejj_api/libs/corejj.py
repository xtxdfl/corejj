...
class Notification:
    def __init__(self, grp, event, target, title, message, duration):
        ...
        self.corejj_push_key = AppSetting.get_default('corejj_push_key')
    ...
    def monitor_by_corejj_push(self, targets):
        if not self.corejj_push_key:
            Notify.make_monitor_notify(
                '发送报警信息失败',
        ...
        self.handle_request(f'{push_server}/corejj/message/', data, 'corejj')
    ...
    def dispatch_monitor(self, modes):
        ...
        if targets:
            self.monitor_by_corejj_push(targets)

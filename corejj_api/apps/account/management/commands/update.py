...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        res = requests.get(f'https://api.corejj.cc/apis/release/latest/?version={version}').json()
        if res['error']:
        ...
        # update web
        web_dir = os.path.join(settings.BASE_DIR, '../corejj_web')
        commands = [
            f'curl -o /tmp/corejj_web.tar.gz https://cdn.corejj.cc/corejj/web_{version}.tar.gz',
            f'rm -rf {web_dir}/build',
            f'tar xf /tmp/corejj_web.tar.gz -C {web_dir}'
        ]

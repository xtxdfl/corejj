...
class MFAView(AdminView):
    def get(self, request):
        ...
        send_login_code(corejj_push_key, request.user.wx_token, code)
...
def get_about(request):
    return json_response({
        ...
        'corejj_version': settings.SPUG_VERSION,
...
def handle_push_bind(request):
    form, error = JsonParser(
        Argument('corejj_push_key', required=False),
    ).parse(request.body)
    if error is None:
        if not form.corejj_push_key:
            AppSetting.delete('corejj_push_key')
            return json_response()

        try:
            res = get_balance(form.corejj_push_key)
        except Exception as e:
            return json_response(error=f'绑定失败：{e}')

        AppSetting.set('corejj_push_key', form.corejj_push_key)
...
def handle_push_balance(request):
    token = AppSetting.get_default('corejj_push_key')
    if not token:

...
def post_request_ext1(request):
    ...
    if error is None:
        ...
        form.corejj_version = Repository.make_corejj_version(deploy.id)
        if form.extra[0] == 'tag':
            if not form.extra[1]:
            ...
            form.corejj_version = repository.corejj_version
...
def post_request_ext1_rollback(request):
    ...
    if error is None:
        ...
        versions = list({x.corejj_version: 1 for x in requests}.keys())
        if req.corejj_version not in versions[:req.deploy.extend_obj.versions + 1]:
...
def post_request_ext2(request):
    ...
    if error is None:
        ...
        if DeployExtend2.objects.filter(deploy=deploy, host_actions__contains='"src_mode": "1"').exists():
            ...
            form.corejj_version = extra['path']

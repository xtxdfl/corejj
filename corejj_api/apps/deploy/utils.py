...
def dispatch(req, fail_mode=False):
    ...
    try:
        ...
        env = AttrDict(
            ...
            SPUG_BUILD_VERSION=req.corejj_version,
...
def _ext1_deploy(req, helper, env):
    if not req.repository_id:
        rep = Repository(
            ...
            version=req.version,
            corejj_version=req.corejj_version,
            extra=req.extra,
...
def _ext2_deploy(req, helper, env):
    ...
    for action in host_actions:
        if action.get('type') == 'transfer':
            ...
            tar_gz_file = f'{req.corejj_version}.tar.gz'
    ...
    if host_actions:
        if req.deploy.is_parallel:
            ...
            with futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                for h_id in req.host_ids:
                    new_env = AttrDict(env.items())
                    t = executor.submit(_deploy_ext2_host, helper, h_id, host_actions, new_env, req.corejj_version)
                    t.h_id = h_id
                    threads.append(t)
                for t in futures.as_completed(threads):
                    exception = t.exception()
            ...
            while host_ids:
                ...
                new_env = AttrDict(env.items())
                try:
                    _deploy_ext2_host(helper, h_id, host_actions, new_env, req.corejj_version)
                    req.fail_host_ids.remove(h_id)
                except Exception as e:
                    helper.send_error(h_id, f'Exception: {e}', False)
                    for h_id in host_ids:
...
def _deploy_ext1_host(req, helper, h_id, env):
    ...
    with host.get_ssh(default_env=env) as ssh:
        ...
        if req.type == '2':
            ...
            command = f'cd {extend.dst_repo} && rm -rf {req.corejj_version} && tar xf {tar_gz_file} && rm -f {req.deploy_id}_*.tar.gz'
            helper.remote_raw(host.id, ssh, command)
            helper.send_step(h_id, 1, '\033[32m完成√\033[0m\r\n')

        # pre host
        repo_dir = os.path.join(extend.dst_repo, req.corejj_version)
...
def _deploy_ext2_host(helper, h_id, actions, env, corejj_version):
            ...
            if action.get('type') == 'transfer':
                if action.get('src_mode') == '1':
                    try:
                        ...
                        callback = helper.progress_callback(host.id)
                        ssh.put_file(os.path.join(REPOS_DIR, env.SPUG_DEPLOY_ID, corejj_version), dst, callback)
                    except Exception as e:
                        helper.send_error(host.id, f'Exception: {e}')
                    ...
                    sp_dir, sd_dst = os.path.split(action['src'])
                    tar_gz_file = f'{corejj_version}.tar.gz'
                    try:
                        callback = helper.progress_callback(host.id)
                    ...
                    command = f'mkdir -p /tmp/{corejj_version} && tar xf /tmp/{tar_gz_file} -C /tmp/{corejj_version}/ '
                    command += f'&& rm -rf {action["dst"]} && mv /tmp/{corejj_version}/{sd_dst} {action["dst"]} '
                    command += f'&& rm -rf /tmp/{corejj_version}* && echo "transfer completed"'
            else:

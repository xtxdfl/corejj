...
def dispatch(rep: Repository, helper=None):
    ...
    if not helper:
        ...
        rds_key = f'{settings.BUILD_KEY}:{rep.corejj_version}'
    ...
    try:
        ...
        env = AttrDict(
            ...
            SPUG_BUILD_VERSION=rep.corejj_version,
        ...
        helper.local(f'cd {REPOS_DIR} && rm -rf {rep.corejj_version}')
...
def _build(rep: Repository, helper, env):
    ...
    build_dir = os.path.join(REPOS_DIR, rep.corejj_version)
    tar_file = os.path.join(BUILD_DIR, f'{rep.corejj_version}.tar.gz')
    if extras[0] == 'branch':
        tree_ish = extras[2]
    ...
    command = f'cd {git_dir} && git archive --prefix={rep.corejj_version}/ {tree_ish} | (cd .. && tar xf -)'
    ...
    filter_rule, exclude, contain = json.loads(extend.filter_rule), '', rep.corejj_version
    ...
    if files:
        if filter_rule['type'] == 'exclude':
            ...
            for x in files:
                if x.startswith('/'):
                    excludes.append(f'--exclude={rep.corejj_version}{x}')
                else:
                    excludes.append(f'--exclude={x}')
            exclude = ' '.join(excludes)
        else:
            contain = ' '.join(f'{rep.corejj_version}/{x}' for x in files)
